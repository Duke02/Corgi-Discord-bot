...
class Database:
    ...
    def add_quote(self, quote: str, author: str, time: tp.Optional[float] = None):
        ...
        self.execute(f'insert into {self.quotes_table_name} values(?, ?, ?)', (quote, author, str(time)))

    def get_random_quote(self):
        with closing(self.get_connection()) as connection:
            with closing(connection.cursor()) as cursor:
                cursor.execute(f'select * from {self.quotes_table_name} order by random() limit 1;')
    ...
    def add_affection(self, user_id: int, delta_affection: int):
        sql_statement: str = f'select {self.affection_column} from {self.relation_table_name} where {self.user_id_column} = ? limit 1;'
        ...
        with closing(self.get_connection()) as connection:
            ...
            if row is None:
                ...
                with closing(connection.cursor()) as cursor:
                    cursor.execute(insert_sql, [user_id, delta_affection, update_time])
            else:
                ...
                with closing(connection.cursor()) as cursor:
                    cursor.execute(
                        f'update {self.relation_table_name} set {self.affection_column} = ?, {self.updated_time_column} = ? where {self.user_id_column} = ?',
    ...
    def get_most_loved(self, top_n: int = 10) -> tp.List[tp.Dict[str, int]]:
        with closing(self.get_connection()) as connection:
            with closing(connection.cursor()) as cursor:
                cursor.execute(
                    f'select {self.user_id_column}, {self.affection_column} from {self.relation_table_name} order by {self.affection_column} desc limit {top_n};')
    ...
    def get_affection(self, user_id: int) -> int:
        with closing(self.get_connection()) as connection:
            with closing(connection.cursor()) as cursor:
                cursor.execute(
                    f'select {self.affection_column} from {self.relation_table_name} where {self.user_id_column} = ? limit 1',
                    [user_id])
                row = cursor.fetchone()
                affection: int = row[0]
                return affection
    ...
    def get_max_affection(self) -> int:
        with closing(self.get_connection()) as connection:
            with closing(connection.cursor()) as cursor:
                ...
                cursor.execute(
                    f'select max({self.affection_column}) as {max_name} from {self.relation_table_name} limit 1')
                row = cursor.fetchone()
